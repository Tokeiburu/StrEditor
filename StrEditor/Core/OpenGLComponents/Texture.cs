...
namespace StrEditor.Core.OpenGLComponents {
	...
	public static class TextureManager {
		...
		public static Texture LoadTexture(string texture, string path, object context) {
			return LoadTexture(texture, path, TextureRenderMode.DefaultTexture, context);
		}

		public static Texture LoadTexture(string texture, string path, TextureRenderMode renderMode, object context) {
			...
			var tuple = new Utilities.Extension.Tuple<Texture, int>(new Texture(texture, new GrfImage(ResourceManager.GetData(path)), true, renderMode), 1);
			_bufferedTextures[texture] = tuple;
			return tuple.Item1;
		}

		public static Texture LoadTexture(string texture, byte[] data, object context) {
			return LoadTexture(texture, data, TextureRenderMode.DefaultTexture, context);
		}

		public static Texture LoadTexture(string texture, byte[] data, TextureRenderMode renderMode, object context) {
			_addContextTexture(context, texture);
			...
			var tuple = new Utilities.Extension.Tuple<Texture, int>(new Texture(texture, new GrfImage(data), true, renderMode), 1);
			_bufferedTextures[texture] = tuple;
			return tuple.Item1;
		}
	...
	public class Texture {
		...
		public Texture(string resource, GrfImage image, bool load = true, TextureRenderMode renderMode = TextureRenderMode.DefaultTexture) {
			IsLoaded = false;
			Resource = resource;
			Image = image;
			IsSemiTransparent = Resource.IsExtension(".tga", ".png");
		...
		public void Set(GrfImage image, TextureRenderMode renderMode = TextureRenderMode.DefaultTexture) {
			if (IsUnloaded)
				return;
		...
		public void Unload(object context) {
			TextureManager.UnloadTexture(Resource, context);
		}
		...
		private void _setTextureMode() {
			switch (RenderMode) {
				case TextureRenderMode.RepeatBackgroundTexture:
				...
				case TextureRenderMode.StrTexture:
					...
					GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureWrapT, (int)TextureWrapMode.Repeat);
					break;
				case TextureRenderMode.DefaultTexture:
					if (EnableMipmap) {
						GL.GenerateMipmap(GenerateMipmapTarget.Texture2D);
					...
					GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureWrapT, (int)TextureWrapMode.ClampToEdge);
					break;
				case TextureRenderMode.NearestNeighbor:
